...
std::string counterPLD = R"(GAL16V8    ; 4-Bit-Counter  first line : used GAL
...
TEST(TestSuite, testCasdfasdfa) {
	std::vector<unsigned char> v(counterPLD.begin(), counterPLD.end());

    int rc = assemblePldMemory("Counter.pld", v.data(), v.size(), 1, 1, 1, 1, 1);


    EXPECT_EQ(rc ,0) << counterPLD.size() << "  \n\n";
}
